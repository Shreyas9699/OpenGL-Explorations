...
public:
	...
	Model(const char* model_path)
	{
		...
		model_directory = std::string(model_path).substr(0, std::string(model_path).find_last_of("\\/"));
...
private:
	void load_model()
	{
		...
		else
		{
			...
			for (unsigned int i = 0; i < num_meshes; ++i)
			{
				...
				mesh_list[i].diffuseColor = glm::vec3(diff.r, diff.g, diff.b);
				mesh_list[i].specularColor = glm::vec3(spec.r, spec.g, spec.b);
				mesh_list[i].hasTexture = false;
				...
				for (unsigned int tex_count = 0; tex_count < material->GetTextureCount(aiTextureType_DIFFUSE); ++tex_count) // Also, only using: aiTextureType_DIFFUSE.
				{
					...
					// *********see if we can use dict instead of vector
					// ---------------------------------------------------------------
	...
	unsigned int load_texture_image(std::string file_name, bool& load_complete)
	{
		...
		std::string full_path = model_directory + "\\" + file_name;
		...
		if (image_data)
		{
			...
			std::cout << "   Image loaded OK: " << file_name << std::endl;
		}
		else
		{
			load_complete = false;
			stbi_image_free(image_data);
			std::cout << "   Image failed to load: " << file_name << std::endl;
		}
		return textureID;
	}
};
